# The code is from https://github.com/voxelmorph/voxelmorph/blob/dev/voxelmorph/torch/networks.py
...
class VxmDense(LoadableModel):
    ...
    def forward(self, source, target, registration=False, inshape=None, all=False):
        '''
        Parameters:
        ...
        if inshape is not None:
            self.transformer = SpatialTransformer(inshape).to(source.device)
            down_shape = [int(dim / 2) for dim in inshape]
            self.integrate = VecInt(down_shape, 7).to(source.device)
        ...
        if all:
            return [y_source, y_target, pos_flow, neg_flow]

        if not registration:
            return [y_source, y_target, preint_flow] if self.bidir else [y_source, preint_flow]
        if registration:
            return [y_source, y_target, pos_flow]
...
class SpatialTransformer(nn.Module):
    ...
    def forward(self, src, flow, deformedGrid=False):
        # new locations
        if not deformedGrid:
        ...
        return nnf.grid_sample(src.float(), new_locs, align_corners=True, mode=self.mode)
...
class Int(nn.Module):
    ...
    def __init__(self, inshape, device):
        super().__init__()
        self.transformer = SpatialTransformer(inshape).to(device)

    def forward(self, a, b):
        return a + self.transformer(a, b)
